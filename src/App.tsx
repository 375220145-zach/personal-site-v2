...
function getWorkData(type: string) {
  if (type === 'donner') return {
    ...
    body: <><ModalH4>核心职责</ModalH4><ModalP>主导 IPD 自研产品全生命周期管理，包括开发进度规划、预算编制、成本审核、风险管理；运用敏捷管理方法推动项目交付。</ModalP><ModalP>独立开发 AI 会议纪要工具（Claude Code），覆盖内容导入、AI 分析、风险识别、待办追踪。</ModalP><ModalH4>核心成果</ModalH4><ModalP><HL>100%</HL> — 项目里程碑及评审准时达成率（非 ECN）</ModalP><ModalP><HL>85.7%</HL> — TR 评审一次通过率</ModalP><ModalP><HL>≤ 3%</HL> — ADCP1 整机成本偏差（PDC 目标成本法）</ModalP><ModalP><HL>~10%</HL> — 敏捷管理使日常项目周期平均缩短</ModalP></>,
  }
  return {
...
function getProjectData(type: string) {
  if (type === 'pm-os') return {
    ...
    body: <><ModalH4>项目简介</ModalH4><ModalP>源自 DONNER 实战经验（IPD 流程、TR/DCP 评审、MIL、BOM），用 Claude Code 独立开发。覆盖概念到量产全过程。</ModalP><div style={{ marginBottom: '4px' }}><a href="https://demo.pm-os.pages.dev" target="_blank" style={{ color: '#DEDBC8', textDecoration: 'none', border: '0.5px solid rgba(222,219,200,0.2)', padding: '4px 10px', borderRadius: '4px', fontSize: '12px' }}>Live Demo ↗ demo.pm-os.pages.dev</a></div><ModalH4>核心功能</ModalH4><ModalP><HL>IPD 全流程</HL> — 13 个里程碑节点（TR1→CDCP→TR2→HMS→TR3→PDCP→EVT→TR4→DVT→TR5→ADCP1→PVT→MP），7 阶段门管理，准入准出标准</ModalP><ModalP><HL>自绘 SVG 甘特图</HL> — 不依赖第三方库，支持缩放、依赖连线、里程碑菱形标记、今日红线、逾期高亮</ModalP><ModalP><HL>9 张数据表</HL> — 项目 / 里程碑 / 任务 / 会议 / 复盘 / 变更 / BOM / 采购 / 认证 / MIL，全部 IndexedDB 本地存储，离线可用</ModalP><ModalP><HL>AI 辅助</HL> — DeepSeek 驱动会议纪要智能分析（行动项提取 + 风险识别）、复盘自动生成（聚合项目数据）</ModalP><ModalP><HL>双部署</HL> — Vercel 全功能可编辑版 + Cloudflare Pages 只读 Demo 版（国内直连）</ModalP><ModalP><HL>Excel 批量导入</HL> — 所有面板支持模板下载 + 批量导入，适配企业级数据迁移</ModalP></>,
  }
...
- 主管多项自研产品项目管理，覆盖 IPD 全过程（CDCP→PDAP→ADCP，HMS→EVT→DVT→PVT→MP）
- 里程碑准时达成率 100%、TR 一次通过率 85.7%、成本偏差 ≤3%、项目周期缩短 ~10%
- 同时管理 10+ 项目，覆盖吉他效果器、吉他音箱、无头吉他、零售空间四大品类

### 管理项目明细
- **吉他效果器** — MIYAVI 联名款 3-in-1（IPD 全流程，周期缩短 21%，成本偏差 1.93%-2.41%，MP 良率 92%+）、Pocket Nano 掌机效果器（IPD 全流程）、HUSH I 迭代、HUSH I PRO 拓展色、HUSH X PRO 拓展色、HUSH X PRO 宝可梦联名款（项目规划、竞品分析、打样测试）
- **吉他音箱** — Pocket Wave（IPD 全流程）、Plugone 音箱电吉他（IPD 全流程）
- **无头吉他** — DST1000（IPD 全流程）、HLX 拓展色、HLX550 迭代（IPD 全流程）
- **零售空间** — 南昌武商 Mall 标杆店（采购寻源、设计转化、打样及出货跟进）、上海华狮旗舰店重建（采购打样、订单及物流管理、现场监工）、RFID 自助收银全国铺设（华南/华中/华东订单及物流规划）
- 0→1 搭建 AI 甘特图自动编排（MIYAVI 项目）
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: loading ? 0 : 1 }} transition={{ duration: 0.8, delay: 0.3 }} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: isMobile ? '0 16px 16px' : '0 0 20px 10px', zIndex: 2 }}>
            {isMobile ? (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: '48px', fontWeight: 700, color: '#DEDBC8', letterSpacing: '-0.03em', lineHeight: 0.9, marginBottom: '16px', transform: 'scaleX(0.88)' }}>Zachary Pan</div>
